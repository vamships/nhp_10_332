...
coxSurvivalFinalModel = function(feats,subj_original,seedMe,scolors,group_colors,num_folds,plots,dir_surv){
  ...
  plot(survfit(fit_cox_all,newdata=colMeans(feats_scaled[which(subj_original[,'groupID']==3),])),mark.time=FALSE,conf.int=FALSE,col=group_colors['3'],lwd=3,xlab='Challenges',ylab='uninfected ratio')
  lines(survfit(fit_cox_all,newdata=colMeans(feats_scaled[which(subj_original[,'groupID']==4),])),mark.time=FALSE,conf.int=FALSE,col=group_colors['4'],lwd=3)
  lines(survfit(fit_cox_all,newdata=colMeans(feats_scaled[which(subj_original[,'groupID']==5),])),mark.time=FALSE,conf.int=FALSE,col=group_colors['5'],lwd=3)
  ...
  km_colors = group_colors
  names(km_colors) = c(1,2,3)
  ...
  fold_feats_betas = matrix(0,nrow=num_folds,ncol=ncol(feats))
  ...
  for(foldIdx in 1:num_folds){
    ...
    fold_feats_betas[foldIdx,] = fit_cox_cv$coefficients
  ...
  barplot(colMeans(fold_feats_betas),main=c(" Predictor Coefficients"),beside=TRUE,las=2,cex.names=0.9,ylim=c(-1,1),names.arg = colnames(feats))
  dev.off()
  ...
  if(plots){
    ...
    diff_g3g4 = wilcox.test(OvP[grp_3,'Risk'],OvP[grp_4,'Risk'],alternative="two.sided")
    diff_g3g5 = wilcox.test(OvP[grp_3,'Risk'],OvP[grp_5,'Risk'],alternative="two.sided")
    diff_g4g5 = wilcox.test(OvP[grp_4,'Risk'],OvP[grp_5,'Risk'],alternative="two.sided")
    
    p = ggplot(OvP, aes(x=Group, y=Risk, color=Group)) + geom_boxplot(width=0.4,notch = F,outlier.shape = NA, na.rm=T, size=1,colour="black") + geom_point(position = position_jitter(w=0.1),size=4,aes(colour=Group)) + scale_x_discrete(labels=names(group_id)) + scale_y_continuous(limits=c(yrange[1],yrange[2]*1.7)) + ylab('Predicted Risk') + theme(axis.line = element_line(colour = "black",size=1), axis.title.x = element_text(size=25,colour='black'), axis.title.y = element_text(size=25,colour='black') ,axis.text.x = element_text(size=20,colour='black'), axis.text.y = element_text(size=15,colour='black'),panel.background = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.y = element_blank(), panel.grid.major.y = element_blank(),aspect.ratio=1, legend.position='none') + xlab('') + scale_colour_manual(values=group_colors)
    p = p + geom_hline(yintercept=0,colour='black',size=0.78,linetype='dashed',alpha=0.7)
    
    p = p + annotate("segment",x=1,xend=2,y=yrange[2]*1.1,yend=yrange[2]*1.1,size=1.5,colour='black')
    p = p + annotate("text",x=1.5,y=yrange[2]*1.18,size=5,label=paste('P : ',format(diff_g3g4$p.value,digits=3,scientific=T)))
    
    p = p + annotate("segment",x=1,xend=3,y=yrange[2]*1.3,yend=yrange[2]*1.3,size=1.5,colour='black')
    p = p + annotate("text",x=2,y=yrange[2]*1.38,size=5,label=paste('P : ',format(diff_g3g5$p.value,digits=3,scientific=T)))
    
    p = p + annotate("segment",x=2,xend=3,y=yrange[2]*1.5,yend=yrange[2]*1.5,size=1.5,colour='black')
    p = p + annotate("text",x=2.5,y=yrange[2]*1.58,size=5,label=paste('P : ',format(diff_g4g5$p.value,digits=3,scientific=T)))
    ...
    p = ggplot(OvP, aes(x=Challenges, y=Risk_Train, color=Group)) + geom_point(size=3) + ggtitle(paste("Challenges vs Predicted Risk (polyserial : ",ppc_train,")",sep="")) + scale_colour_manual(name='Groupwise Coeff\nPolyserial',values=group_colors)
    print(p)
    dev.off()
