...
coxSurvivalLookBack = function(feats,feats_all,top_feat_idx,lookBackCorThresh,baselineCindex,seedMe,subj_original,scolors_original,lcolors_all,group_colors,num_folds,log_file,dir_lookBack){
    ...
    if(length(otherFeats)>1){
      ...
      for(otherFeatIdx in otherFeats){
        ...
        subColors[otherFeatCount] = lcolors_all[otherFeatIdx,1]
          ...
          model_add = coxSurvivalFinalModel(feats_add,subj_original,seedMe,scolors_original,group_colors,num_folds,plots=TRUE,dir_otherFeat)
