...
# Project     : NIH 10-332
# Decription  : Convert survival probabilities into event tables using samples
# Cite        : TBD
# ******************************************************************************
...
convertKMtoEvents = function(KMprobs,max_chall,num_samples){
  ...
  for(stepsIdx in 1:max_chall){
    ...
    #cat('step : ',stepsIdx,'start : ',start_idx+1,' end : ',end_idx[stepsIdx],'\n')
    
    if(surv_table[stepsIdx,2]!=0){
      event_table[(start_idx+1):end_idx[stepsIdx],1] = stepsIdx
      #       cat('assigned :',event_table[(start_idx+1):end_idx[stepsIdx],1],'\n')
      #       cat(event_table[,1],'\n')
    }
